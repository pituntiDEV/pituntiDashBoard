...
import { useGetEmbySharedServers } from '../../../../hook/jellyfin/useGetEmbySharedServers';
                        ...
                        servers.map(account => {
                            return (
                                <option data-admin={account.admin} value={account._id} key={account._id}>
                                    {account.data.name}-({`${account.data?.ip}:${account.data?.port}`})
                                </option>
                            )
                        })
                    }
                        ...
                        sharedServers.map(server => {
                            return (
                                <option data-admin={server.server.admin} data-shared={true} value={server.server._id} key={server.server._id}>
                                    {server.server.data.name}-(Compartido)-({`${server.server?.data?.ip}:${server.server?.data?.port}`})
                                </option>
                            )
                        })
                    }
